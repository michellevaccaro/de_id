...
def createConversionDict(val,num,denom,numDictName):
    """
...
def main(c, year_bin_file, post_bin_file):
    ...
    while sum(denom<k)>1:
        val,num,denom = collapse(val,num,denom)
    # Create dictionary that maps every unique value to a corresponding bin, and add the empty values to it
    dictpost = createConversionDict(val,num,denom,post_bin_fname)
